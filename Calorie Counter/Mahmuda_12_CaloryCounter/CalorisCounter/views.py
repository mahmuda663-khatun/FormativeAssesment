...
def dashboard_view(request):
    # ১. ইউজারের প্রোফাইল আছে কি না চেক করি। না থাকলে প্রোফাইল তৈরি করতে পাঠাই।
    if not hasattr(request.user, 'profile'):
        return redirect('profile')
    
    profile = request.user.profile
    today = timezone.now().date() # আজকের তারিখ

    # ২. আজকের সব খাবারের লিস্ট বের করি (ফিল্টার করে)
    entries = FoodEntry.objects.filter(user=request.user, date=today)

    # ৩. মোট কত ক্যালরি খাওয়া হয়েছে তা হিসাব করি (সহজ লুপ দিয়ে)
    total_calories = 0
    for food in entries:
        total_calories += food.calories

    # ৪. টেমপ্লেটে ডাটা পাঠানোর জন্য ডিকশনারি সাজাই
    context = {
        'profile': profile,
        'entries': entries,
        'total_consumed': total_calories,
